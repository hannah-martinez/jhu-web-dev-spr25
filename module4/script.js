...
const namesWithGreetings = names.map( (name) => speakSimple(name) );
for (i = 0; i < namesWithGreetings.length; i++) {
  console.log(namesWithGreetings[i]);
}

// #3
let initialValue = {hello: [], bye: []};
let separateNames = names.reduce(
...
for (i = 0; i < separateNames.hello.length; i++) {
  helloSpeaker.speak(separateNames['hello'][i]);
}
for (i = 0; i < separateNames.bye.length; i++) {
  byeSpeaker.speak(separateNames['bye'][i]);
}
